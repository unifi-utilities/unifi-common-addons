...
BINARY=$APP_HOME/current/soundtouch-service
HEALTHCHECK=${AFTERTOUCH_HEALTHCHECK:-$APP_HOME/aftertouch-service-healthcheck.sh}

die() {
	echo "aftertouch-service boot: ERROR: $*" >&2
	exit 1
}
...
for path in "$APP_HOME/config.env" "$APP_HOME/run.sh" "$APP_HOME/aftertouch-service.service" "$HEALTHCHECK" "$MANIFEST" "$BINARY"; do
...
mkdir -p "$DATA_DIR"
chmod 0700 "$DATA_DIR"
"$CHOWN" "$SERVICE_UID:$SERVICE_GID" "$DATA_DIR"
chmod 0600 "$APP_HOME/config.env"
"$CHOWN" 0:0 "$APP_HOME/config.env" "$APP_HOME/run.sh" \
	"$APP_HOME/aftertouch-service.service" "$HEALTHCHECK"
...
if [ -L "$APP_HOME/verified" ]; then
	...
	if release_target_valid "$verified_candidate"; then
		...
		echo "aftertouch-service boot: WARNING: ignoring corrupt verified target: $verified_candidate" >&2
	fi
fi

mkdir -p "$UNIT_DIR"
install -m 0644 "$APP_HOME/aftertouch-service.service" "$UNIT_DIR/$SERVICE_NAME"
"$SYSTEMCTL" daemon-reload
# udm-boot owns boot ordering; remove legacy enablement to avoid a parallel start.
"$SYSTEMCTL" disable "$SERVICE_NAME" >/dev/null
"$SYSTEMCTL" restart "$SERVICE_NAME"
AFTERTOUCH_SERVICE_HOME=$APP_HOME "$HEALTHCHECK"
