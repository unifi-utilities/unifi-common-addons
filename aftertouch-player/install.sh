...
die() {
	echo "aftertouch-player install: ERROR: $*" >&2
	exit 1
}
...
case "$APP_HOME" in
/*) ;;
*) die "AFTERTOUCH_PLAYER_HOME must be an absolute path" ;;
esac
[ "$APP_HOME" != / ] || die "refusing to use / as AFTERTOUCH_PLAYER_HOME"
case "$ON_BOOT_DIR" in
/*) ;;
...
mkdir -p "$APP_HOME" "$APP_HOME/releases" "$ON_BOOT_DIR"
...
for required in run.sh aftertouch-player-healthcheck.sh aftertouch-player.service 26-aftertouch-player.sh aftertouch-player.env.example; do
	[ -f "$SCRIPT_DIR/$required" ] || die "missing addon file: $SCRIPT_DIR/$required"
done

copy_file "$SCRIPT_DIR/run.sh" "$APP_HOME/run.sh" 0755
copy_file "$SCRIPT_DIR/aftertouch-player-healthcheck.sh" "$APP_HOME/aftertouch-player-healthcheck.sh" 0755
copy_file "$SCRIPT_DIR/aftertouch-player.service" "$APP_HOME/aftertouch-player.service" 0644

if [ ! -f "$APP_HOME/config.env" ]; then
	install -m 0640 "$SCRIPT_DIR/aftertouch-player.env.example" "$APP_HOME/config.env"
	echo "aftertouch-player install: created $APP_HOME/config.env; review it before activation"
fi
...
stage_artifact() {
	...
	install -m 0755 "$stage_source_file" "$STAGE_DIR/soundtouch-player"
	stage_actual=$(sha256sum "$STAGE_DIR/soundtouch-player" | awk '{print $1}')
...
EOF
	...
	if [ -d "$stage_release_dir" ]; then
		...
		stage_installed=$(sha256sum "$stage_release_dir/soundtouch-player" | awk '{print $1}')
	...
	echo "aftertouch-player install: selected $stage_artifact_id ($stage_actual)"
}

case "$MODE" in
release)
	asset=soundtouch-player-${VERSION}-${ARCH}
...
rollback)
	...
	if [ -L "$APP_HOME/verified" ]; then
		...
		if valid_release_target "$verified_target" &&
			...
			echo "aftertouch-player install: WARNING: ignoring corrupt verified target: $verified_target" >&2
		fi
	fi
	...
	echo "aftertouch-player install: selected rollback target $rollback_target"
	;;
esac
...
if [ "$ACTIVATE" -eq 1 ]; then
	copy_file "$SCRIPT_DIR/26-aftertouch-player.sh" "$ON_BOOT_DIR/26-aftertouch-player.sh" 0755
	AFTERTOUCH_PLAYER_HOME=$APP_HOME AFTERTOUCH_OPERATION_LOCK_HELD=1 \
		"$ON_BOOT_DIR/26-aftertouch-player.sh"
fi

if [ "$ACTIVATE" -ne 1 ]; then
	echo "aftertouch-player install: artifact selected but service not restarted; use --activate after reviewing config.env"
	if [ -e "$ON_BOOT_DIR/26-aftertouch-player.sh" ]; then
		echo "aftertouch-player install: WARNING: an existing boot hook will use the selected artifact after a reboot" >&2
	fi
fi
